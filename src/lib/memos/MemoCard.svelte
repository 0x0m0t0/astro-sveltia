<script lang="ts">
	...
	let images = $derived(
		(memo.attachments || []).filter(isImage).map((a) => ({
			src: getFileUrl(a.name, a.filename, apiUrl),
			alt: a.filename
		}))
	)
</script>
...
<article
	class="bg-white dark:bg-white/5 transition-colors duration-500 p-2 border-b border-darkSand/10 dark:border-darkText/10"
	data-memo-id={memo.name || memo.uid}
>
